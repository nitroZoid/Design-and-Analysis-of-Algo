...
int order(pair<int,int> a, pair<int,int> b, pair<int,int> c){
	...
	if(det == 0) return 0;           //if points are collinear
	if(det < 0)  return 1;           //if a,b,c are clockwise 
	return 2;						 //if a,b,c are anti-clockwise
}

double getDist(pair<int, int> p0, pair<int, int> a, pair<int, int> b){
	double m = (double)((b.y-a.y)*1.0)/(b.x-a.x);
	double c = a.y-m*a.x;			//equation of line -> y = mx+c
...
void qhull(vector<pair<int,int> > p, pair<int, int> a, pair<int, int> b, vector<pair<int, int> >&res){
	...
	//Divide the points into three sets
	//Ignore points inside the triangle
...
void convexHull(vector<pair<int,int> > p){
	...
	//Get the leftmost and rightmost point - the two extremes
	int id1 = 0, id2 = 0;
	for(int i = 1;i < n;i++){
